<?php 
    ...
    {
        // Xử lý nếu giỏ hàng có dữ liệu (code hiện tại chưa làm gì tại đây)
    }

    // Kiểm tra nếu người dùng đã đăng nhập
    if($_SESSION['dangnhapuser'])
    {
        // Lấy thông tin tài khoản và ID người dùng từ session
        $account_user = $_SESSION['dangnhapuser'];
        $id_user = $_SESSION['id_user'];
    }

    // Truy vấn lấy thông tin giao hàng của người dùng từ bảng shipping
    $sql_user_shipping = mysqli_query($mysqli,"SELECT * from shipping where id_user='$id_user' limit 1");
    $row_user_shipping = mysqli_fetch_array($sql_user_shipping); // Lưu kết quả truy vấn vào mảng
?>
